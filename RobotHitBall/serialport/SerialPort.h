...
{
	AvoidCarKinect = -2,//另一台车看到球，求出目标位置
	AvoidCarTH = -1,//双目看到球，指挥另一台车避让
	Wait = 0,
	GetTheBallKinect = 1,
	GetTheBallTH = 2,
	GetTheBallLA = 3,
...
typedef enum tagFramwType
{
	...
	AimPosFrame = 1,//控制移动帧// {0xFF 帧头 信息 x ，y t } //信息包括 A高球0 A车低球1 B车高球2 B车低球3
	AskTypeFrame = 2,
	AnswerTypeFrame = 3, //帧定义 ：{ 0XFF, 帧头，归属 ，x，y，t } ）
	ResetFrame = 4,
} FramwType;
/*******************************************************************/
...
public:
	...
	//发送队列
	//发送队列中，第一位为标志位 0 或则1 1表示待发送 0表示已发送
